...
export const obterSaldoUsuario = async (req: any, res: any) => {
  const { codUsuario } = req.params;
  ...
  try {
    const data = await withConnection(async (connection) => {
      const sql = `
        SELECT 
          u.CODSETOR,
          s.DESCRICAO AS SETOR,
          NVL(s.SALDO, 0) AS SALDO
        FROM BRAMV_USUARIOS u
        LEFT JOIN BRAMV_SETOR s ON s.CODSETOR = u.CODSETOR
        WHERE u.CODUSUARIO = :id
      `;
      const r = await connection.execute(sql, { id }, { outFormat: oracledb.OUT_FORMAT_OBJECT });
    ...
    console.error('[obterSaldoUsuario] Oracle error:', { errorNum: err?.errorNum, message: err?.message });
