...
export const listarPendentes = async (_req: any, res: any) => {
  try {
    const pedidos = await withConnection(async (connection) => {
      const sql = `
        WITH OrderTotals AS (
          SELECT NUMPEDRCA, COUNT(CODPROD) AS QTD_ITENS, SUM(NVL(QT,0) * NVL(PVENDA,0)) AS VALOR_TOTAL
          FROM BRAMV_PEDIDOI
          GROUP BY NUMPEDRCA
        ),
        UserLatest AS (
          SELECT u.*, ROW_NUMBER() OVER (PARTITION BY u.CODUSUARIO ORDER BY NVL(u.ATUALIZADO_EM, u.CODUSUARIO) DESC) AS RN
          FROM BRAMV_USUARIOS u
        )
        SELECT 
          p.NUMPEDRCA AS ID, p.DATA AS DATA,
  ...
  try {
    const pedidos = await withConnection(async (connection) => {
      const sql = `
        ...
        FROM BRAMV_PEDIDOC p
        LEFT JOIN UserLatest ul  ON ul.CODUSUARIO = p.CODUSUARIO AND ul.RN = 1
        LEFT JOIN BRAMV_SETOR s ON ul.CODSETOR = s.CODSETOR
