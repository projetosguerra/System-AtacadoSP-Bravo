...
import { gateProdutos } from '../utils/gates.js';
...
export const listarProdutos = async (req: any, res: any) => {
  const SAT_THRESHOLD = Number(process.env.PRODUTOS_QUEUE_THRESHOLD ?? 8);
  if (gateProdutos.isSaturated(SAT_THRESHOLD)) {
    res.set('Retry-After', '3');
    return res.status(503).json({ error: 'Busy, try again shortly.' });
  }

  return gateProdutos.run(async () => {
    try {
