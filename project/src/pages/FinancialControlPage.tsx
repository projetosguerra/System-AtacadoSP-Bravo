...
const FinancialControlPage: React.FC = () => {
  ...
  // *** Cálculo corrigido do gasto do setor selecionado usando pedidos aprovados reais
  const selectedData = useMemo(() => {
    ...
    // Soma de VALOR_TOTAL dos pedidos aprovados retornados (query já filtra STATUS=1)
    const gastoReal = sectorOrders.reduce(
      (sum, o) => sum + Number(o.VALOR_TOTAL || 0),
      0
    );

    const limite = setorRef.SALDO || 0; // SALDO aqui representa o limite total configurado
    let disponivel = limite - gastoReal;
    if (disponivel < 0) disponivel = 0; // proteção caso limite seja menor que consumo
  ...
  // *** KPIs do setor selecionado agora usam gastoReal calculado pelos pedidos aprovados
    ...
    return (
      <div className="p-8">
        ...
        <p className="text-sm text-red-600">
          Sua unidade administrativa não foi localizada. Contate o administrador.
        </p>
      </div>
    );
  }
        ...
        {viewMode === 'ALL' && isAdmin ? (
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {allUnitsRows.map(row => (
                      <tr key={row.codsetor} className="hover:bg-gray-50">
                        <td className="px-4 py-3">{row.codsetor} - {row.descricao}</td>
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              {isHistoryLoading ? <p>Carregando detalhes...</p> : (
                <div className="space-y-8">
                  <div>
                    ...
                    <table className="min-w-full text-sm">
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {limitHistory.map((entry, index) => (
                          <tr key={index}>
                            <td className="px-4 py-3">{formatDate(entry.DATA_ALT)}</td>
                            <td className="px-4 py-3 font-mono text-gray-600">{formatTime(entry.DATA_ALT)}</td>
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {sectorOrders.map(order => (
                          <tr key={order.NUMPEDRCA}>
                            ...
                            <td className="px-4 py-3">{formatDate(order.DATA)}</td>
                            <td className="px-4 py-3 font-mono text-gray-600">{formatTime(order.DATA)}</td>
